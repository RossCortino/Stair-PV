function [thigh_mean,knee_mean,ankle_mean,thigh_sd, knee_sd, ankle_sd, ...
    thigh_torque,knee_torque,ankle_torque,thigh_torque_sd, knee_torque_sd, ankle_torque_sd] = averageJointKinematics_Run(Normalized, sub, trial, incline)
...
%     sub{s}
    temp_task = Normalized.(sub{s}).Run.(trial{1}).(incline{1}); 
