...
knee_interp = interp1(pv,knee_mean,T(pv),'spline','extrap');
ankle_interp = interp1(pv, ankle_mean, T(pv),'spline','extrap');
...
plot(pv,ankle_mean)
hold on
plot(pv,ankle_interp,'--')
xlabel('Phase')
legend('Orig','Interp Signal')
...
N = 150
krmse = sqrt(sum((knee_est'-knee_ref).^2)/N)
armse = sqrt(sum((ankle_est-ankle_ref).^2)/N)

%% Duplicate with random example