...
k_new = stairVCTrajectories.knee;
a_new = stairVCTrajectories.ankle;



figure
sgtitle('Check Knee Traj')
% subplot(221)
% plot(t,k_new.i0)
% hold on
% plot(t,knee_0_incvc,'--')
% legend('Stored','Original(0^o)')
subplot(221)
plot(t,k_new.i20)
hold on
plot(t,knee_20_incvc,'--')
legend('Stored','Original(20^o)')
subplot(222)
plot(t,k_new.i25)
hold on
plot(t,knee_25_incvc,'--')
legend('Stored','Original(25^o)')
subplot(223)
plot(t,k_new.i30)
hold on
plot(t,knee_30_incvc,'--')
legend('Stored','Original(30^o)')
subplot(224)
plot(t,k_new.i35)
hold on
plot(t,knee_35_incvc,'--')
legend('Stored','Original(35^o)')
...
[thigh_mean,knee_mean,ankle_mean] = averageJointKinematics_Stair(Normalized,sub,trial,incline);
...
for i = 1:length(thigh_stream)
    ...
    [currPV,currState,sm,qhm,mhf] = calculatePhaseVariable_Stair(thigh, thighd, qh_min, qh_max,q_po, c, prevState,prevPV, sm, qhm,mhf,.69);
    s(i) = currPV;
    prevState = currState;
    state(i) = currState;
    prevPV = currPV;
    ...
    if qh_max ~= temp_max
        xq = qh_max;
        hk_incvc = .5*interp1(x,vkreal_incvc(1,:),xq,'linear','extrap')+.5*interp1(x,vkreal_incvc(N_k/2+1,:),xq,'linear','extrap')*cos(pi*N_k*sh);
        ha_incvc = .5*interp1(x,vareal_incvc(1,:),xq,'linear','extrap')+.5*interp1(x,vareal_incvc(N_a/2+1,:),xq,'linear','extrap')*cos(pi*N_a*sh);
        
        hk_inc = .5*interp1(x,vkreal_inc(1,:),xq,'linear','extrap')+.5*interp1(x,vkim_inc(N_k/2+1,:),xq,'linear','extrap')*cos(pi*N_k*sh);
        ha_inc = .5*interp1(x,vareal_inc(1,:),xq,'linear','extrap')+.5*interp1(x,vaim_inc(N_a/2+1,:),xq,'linear','extrap')*cos(pi*N_a*sh);
        ...
        for k = 1:N_k/2-1
            ...
            hk_test = [hk_test pk_real(k+1) -pk_im(k+1)];
            k+1
        end
        
            hk_test = [hk_test pk_real(N_k/2+1)];
...
estimatedAngles.knee.(incline{1})  = knee_incvc_est;
estimatedAngles.ankle.(incline{1})  = ankle_incvc_est;
estimatedAngles.phase.(incline{1}) = s;
