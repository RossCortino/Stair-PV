...
vk_real = [pk_real20(1:N/2) pk_real25(1:N/2) pk_real30(1:N/2) pk_real35(1:N/2)]';
vk_im = [pk_im20(1:N/2) pk_im25(1:N/2) pk_im30(1:N/2) pk_im35(1:N/2)]';
x = [1:N/2 1:N/2  1:N/2  1:N/2]';
y = repelem([max(thigh_20);max(thigh_25); max(thigh_30); max(thigh_35)],N/2);
...
knee_20_est = double(subs(hk, sh, s_h));
...
knee_30_est = double(subs(hk, sh, s_h));
...
knee_35_est = double(subs(hk, sh, s_h));


% figure
% plot(percentGait,knee_20,percentGait, knee_25, percentGait, knee_30, percentGait, knee_35)
% legend('20','25','30','35')

figure
subplot(2,2,1)
plot(percentGait, knee_20, percentGait, knee_20_est)
title("Knee Angle at 20^{o} Decline")
xlabel("Gaitcycle(%)")
ylabel("Angle(^{o})")
...
legend('Knee','Estimated Knee (Interp Coefficients)')
subplot(2,2,3)
plot(percentGait, knee_30, percentGait, knee_30_est)
title("Knee Angle at 30^{o} Decline")
xlabel("Gaitcycle(%)")
ylabel("Angle(^{o})")
legend('Knee','Estimated Knee (Interp Coefficients)')
subplot(2,2,4)
plot(percentGait, knee_35, percentGait, knee_35_est)
title("Knee Angle at 35^{o} Decline")
xlabel("Gaitcycle(%)")
ylabel("Angle(^{o})")
legend('Knee','Estimated Knee (Interp Coefficients)')
